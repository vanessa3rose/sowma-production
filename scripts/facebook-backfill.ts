...
/* -------------------------------------------------
   Facebook API fetchers (daily only)
-------------------------------------------------- */
  ...
  for (const metric of metrics) {
    ...
    out[metric] =
      metric === "page_follows"
        ...
        : values.reduce((s: number, v: any) => s + (v.value ?? 0), 0);
  }
...
async function fetchDailyPostMetrics(date: Date) {
  ...
  let url =
    `https://graph.facebook.com/${FB_API_VERSION}/${FACEBOOK_PAGE_ID}/posts` +
...
export async function runDailyFacebookSync() {
  ...
  console.log(
    `[FB] Backfilling from ${formatISODate(currentDate)} to ${formatISODate(
      today,
    )}`,
  );
      ...
      await prisma.socialMediaMetrics.createMany({
        data: [
          ...
          },
          {
            ...
            metricValue:
              insights.page_actions_post_reactions_like_total ?? 0,
            metricDate: currentDate,
          },
          {
            socialMediaId: account.id,
